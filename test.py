...
def detect_face(img, tmp_shrink):
    ...
    x_contrast = torch.empty(
        size = (x.shape[ 0 ] , x.shape[ 1 ] , x.shape[ 2 ] , x.shape[ 3 ]) ).cuda()
    net.eval()
    y = net.test_forward(x_dark = x, x_light = x_contrast)
...
def load_models():
    print('build network')
    net = build_net('test', num_classes=1, model='dark')
    net.eval()
    net.load_state_dict(torch.load('./weights/test.pth')) # Set the dir of your model weight
